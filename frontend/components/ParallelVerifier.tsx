...
const initState = (): State =>
  Object.fromEntries(
    CIRCUIT_LIST.map((id) => [
      id,
      { selected: ["execution", "transferVerify", "merkleUpdate"].includes(id), status: "idle" },
    ])
  ) as State;
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case "toggle":
      return { ...state, [action.id]: { ...state[action.id], selected: !state[action.id].selected } };
    case "reset":
      return initState();
    case "start": {
      const next = { ...state };
...
export default function ParallelVerifier() {
  ...
  const run = async () => {
    setLogs([]); setSummary(null); setProgress(0);
    setRunning(true); dispatch({ type: "start", ids: selected });
    const t0 = performance.now();

    const items = selected.map((id) => ({ id, paths: getCircuitPaths(id) }));
    const results = await verifyManyCircuits(items, {
      log: appendLog,
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5 rounded-3xl bg-gradient-to-b from-white to-neutral-50 p-8 shadow-lg">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-neutral-500">v1.0.4-Full • Parallel Mode</span>
      </div>
      ...
      <div className="space-y-2">
        ...
        {summary && (
          <div className="text-sm text-neutral-700">
            Result: <b>{summary.ok}/{summary.total}</b> verified • Elapsed <b>{summary.ms} ms</b>
          </div>
        )}
      </div>
